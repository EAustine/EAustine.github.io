...
-- anonymous, so these are the only guards there are: two image types and an
-- 8 MB ceiling.
--
-- WebP is allowed because PNG is lossless and hopeless for photographs. A
-- screenshot uploads as PNG; a photo that will not fit is re-encoded as WebP
-- by the app before it is sent.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('matte-shares', 'matte-shares', true, 8388608,
...
-- guessing.
--
-- Already run this once? Run it again. The insert upserts, so the new mime
-- list replaces the old one without touching anything already stored.
